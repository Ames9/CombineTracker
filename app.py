...
# ────────────────────────────────────────────────────────────────────────────
with tab_pro:
    _pr_merged  = build_merged_edge_data()
    _pr_season  = build_merged_edge_season_data()

    if _pr_merged.empty and _pr_season.empty:
        st.warning(T["pro_no_file"])
    else:
        # ── ヘッダー ──────────────────────────────────────────────────────────
        st.markdown(f"### {T['pro_title']}")
        _hd1, _hd2, _hd3 = st.columns([3, 1, 1])
        with _hd1:
            st.caption(T["pro_caption"])
        with _hd2:
            st.metric(T["pro_matched"], f"{len(_pr_merged):,}")
        with _hd3:
            st.metric("Player-Seasons", f"{len(_pr_season):,}")

        st.markdown("---")

        # ── ビューモード & 集計モード ─────────────────────────────────────────
        _vcol1, _vcol2 = st.columns([3, 3])
        with _vcol1:
            _view_opts = ["📊 散布図  (Combine → Pro)", "🗺️ 2Dヒートマップ  (Combine × Combine → Pro)"]
            _view_mode = st.radio("表示モード", _view_opts, horizontal=True, index=0)
            _is_heatmap = "ヒートマップ" in _view_mode
        with _vcol2:
            _mode_opts = ["📊 通算 (Career)", "📅 シーズン別 (Per Season)"]
            _mode = st.radio("集計モード", _mode_opts, horizontal=True, index=0)
            _is_season = "Season" in _mode or "シーズン" in _mode

        # ── 共通ラベル ────────────────────────────────────────────────────────
        _ecm_labels = list(_ECM.keys())
        _ppm_labels = list(_PPM_SEASON.keys()) if _is_season else list(_PPM.keys())

        # ════════════════════════════════════════════════════════════════════
        if _is_heatmap:
        # ════════════════════════════════════════════════════════════════════
            # ── ヒートマップ コントロール ─────────────────────────────────────
            _hm1, _hm2, _hm3 = st.columns([3, 3, 3])
            with _hm1:
                _hx_label = st.selectbox("X軸: Combine指標①", _ecm_labels,
                                         index=_ecm_labels.index("Arm Length (in)"))
            with _hm2:
                _hy_label = st.selectbox("Y軸: Combine指標②", _ecm_labels,
                                         index=_ecm_labels.index("40-Yard Dash (s)"))
            with _hm3:
                _hz_label = st.selectbox("色: Pro指標", _ppm_labels, index=0)

            _hm4, _hm5, _hm6, _hm7 = st.columns([2, 2, 2, 2])
            with _hm4:
                _use_pd_hm = st.toggle(T["pro_use_pd"], value=True, key="hm_pd")
            with _hm5:
                _hm_snaps_def = 50 if _is_season else 200
                _hm_min_snaps = st.slider(T["pro_min_snaps"], 0,
                                          200 if _is_season else 600,
                                          _hm_snaps_def, 25 if _is_season else 50,
                                          key="hm_snaps")
            with _hm6:
                _hm_pos = st.multiselect(T["pro_pos"], ["DE","OLB"],
                                         default=["DE","OLB"], key="hm_pos")
            with _hm7:
                _hm_yr = st.slider(T["pro_draft_yr"], 2006, 2024,
                                   (2008, 2022), key="hm_yr")

            _hm_bins_col, _hm_agg_col, _hm_scatter_col = st.columns([2, 2, 2])
            with _hm_bins_col:
                _n_bins = st.slider("グリッドサイズ (N×N)", 5, 25, 12, 1)
            with _hm_agg_col:
                _agg_func_label = st.radio("集計方法", ["平均 (mean)", "中央値 (median)"],
                                           horizontal=True)
                _agg_func = "mean" if "mean" in _agg_func_label else "median"
            with _hm_scatter_col:
                _show_scatter_overlay = st.toggle("個別選手を重ねる", value=True)

            # ── ヒートマップ データ準備 ────────────────────────────────────────
            if _is_season:
                _hdf = _pr_season.copy()
                _hz_col, _hz_lower = _PPM_SEASON[_hz_label]
                _hm_snaps_col = "pass_rush_snaps"
            else:
                _hdf = _pr_merged.copy()
                _hz_col, _hz_lower = _PPM[_hz_label]
                _hm_snaps_col = "total_pr_snaps"

            # X resolve
            _hx_c, _hx_p = _ECM[_hx_label]
            if _hx_c and _hx_c in _hdf.columns:
                _hdf["_hx"] = _hdf[_hx_c]
                if _use_pd_hm and _hx_p and _hx_p in _hdf.columns:
                    _hdf["_hx"] = _hdf["_hx"].combine_first(_hdf[_hx_p])
            else:
                _hdf["_hx"] = np.nan

            # Y resolve
            _hy_c, _hy_p = _ECM[_hy_label]
            if _hy_c and _hy_c in _hdf.columns:
                _hdf["_hy"] = _hdf[_hy_c]
                if _use_pd_hm and _hy_p and _hy_p in _hdf.columns:
                    _hdf["_hy"] = _hdf["_hy"].combine_first(_hdf[_hy_p])
            else:
                _hdf["_hy"] = np.nan

            _hdf["_hz"] = _hdf[_hz_col] if _hz_col in _hdf.columns else np.nan

            _hm_pos_f = _hm_pos if _hm_pos else ["DE","OLB"]
            _hdf = _hdf[
                _hdf["_hx"].notna() &
                _hdf["_hy"].notna() &
                _hdf["_hz"].notna() &
                (_hdf["_hz"] > 0) &
                (_hdf[_hm_snaps_col] >= _hm_min_snaps) &
                _hdf["position"].isin(_hm_pos_f) &
                _hdf["year"].between(_hm_yr[0], _hm_yr[1])
            ].copy()

            _hn = len(_hdf)
            if _hn < 10:
                st.warning(T["pro_no_data"].format(n=_hn))
            else:
                _hx_arr = _hdf["_hx"].values.astype(float)
                _hy_arr = _hdf["_hy"].values.astype(float)
                _hz_arr = _hdf["_hz"].values.astype(float)

                # ── Binned heatmap (scipy) ────────────────────────────────────
                from scipy.stats import binned_statistic_2d

                _stat, _xe, _ye, _ = binned_statistic_2d(
                    _hx_arr, _hy_arr, _hz_arr,
                    statistic=_agg_func,
                    bins=_n_bins,
                )
                # セル中心
                ...
                # Count per bin (for annotation + opacity)
                _cnt, _, _, _ = binned_statistic_2d(
                    _hx_arr, _hy_arr, _hz_arr,
                    statistic="count", bins=_n_bins,
                )

                # サンプル数が少ないセルを NaN に
                _min_cell = max(2, _hn // (_n_bins * _n_bins * 2))
                _stat_masked = np.where(_cnt >= _min_cell, _stat, np.nan)

                # カラースケール (lower=better なら反転)
                _cscale = "RdYlGn_r" if _hz_lower else "RdYlGn"

                # ── Figure ────────────────────────────────────────────────────
                _hfig = go.Figure()

                # Heatmap layer
                _hfig.add_trace(go.Heatmap(
                    x=_xc,
                    y=_yc,
                    z=_stat_masked.T,
                    colorscale=_cscale,
                    ...
                    colorbar=dict(
                        title=dict(
                            text=_hz_label.split("(")[0].strip(),
                            font=dict(color=THEME["label"], size=11),
                        ),
                        thickness=14,
                    ...
                    hovertemplate=(
                        f"{_hx_label}: %{{x:.3f}}<br>"
                        f"{_hy_label}: %{{y:.3f}}<br>"
                        f"{_agg_func} {_hz_label.split('(')[0].strip()}: %{{z:.4f}}"
                        "<extra></extra>"
                    ),
                ))

                # Scatter overlay
                if _show_scatter_overlay:
                    _sc_text = (
                        _hdf["_match_name"] + "<br>"
                        + "Draft " + _hdf["year"].astype(str) + " | "
                        + _hdf["position"] + "<br>"
                        + f"{_hz_label.split('(')[0].strip()}: "
                        + _hdf["_hz"].round(4).astype(str)
                    )
                    _hfig.add_trace(go.Scatter(
                        ...
                        marker=dict(
                            color=_hz_arr,
                            colorscale=_cscale,
                            cmin=float(np.nanmin(_stat_masked)),
                            cmax=float(np.nanmax(_stat_masked)),
                            size=6,
                            opacity=0.6,
                ...
                # 中央値クロスヘア
                _hfig.add_vline(
                    x=float(np.median(_hx_arr)),
                    line=dict(color=THEME["muted"], width=1, dash="dot"),
                    annotation_text=f"med {np.median(_hx_arr):.2f}",
                    annotation_position="top right",
                    annotation_font=dict(size=9, color=THEME["muted"]),
                )
                _hfig.add_hline(
                    y=float(np.median(_hy_arr)),
                    line=dict(color=THEME["muted"], width=1, dash="dot"),
                    annotation_text=f"med {np.median(_hy_arr):.3f}",
                    annotation_position="right",
                    annotation_font=dict(size=9, color=THEME["muted"]),
                )

                _hfig.update_layout(**make_plot_layout(
                    height=600,
                    margin=dict(t=30, b=60, l=70, r=20),
                ))
                _hfig.update_xaxes(title_text=_hx_label)
                _hfig.update_yaxes(title_text=_hy_label)

                st.plotly_chart(_hfig, use_container_width=True)

                # ── ヒートマップ 統計サマリー ──────────────────────────────────
                _hsc1, _hsc2, _hsc3, _hsc4 = st.columns(4)
                _hsc1.metric("選手数 (n)", _hn)
                _hsc2.metric("有効セル数", int(np.sum(~np.isnan(_stat_masked))))
                _hsc3.metric(f"{_agg_func} {_hz_label.split('(')[0].strip()}",
                             f"{np.nanmean(_hz_arr):.4f}")
                _hsc4.metric("中央値", f"{np.nanmedian(_hz_arr):.4f}")
        ...
        # ════════════════════════════════════════════════════════════════════
            ...
            with _pc1:
                _x_label = st.selectbox(T["pro_x"], _ecm_labels,
                                        index=_ecm_labels.index("Arm Length (in)"))
            with _pc2:
                _y_label = st.selectbox(T["pro_y"], _ppm_labels, index=0)
            with _pc3:
                _use_pd_pro = st.toggle(T["pro_use_pd"], value=True)

            _pc4, _pc5, _pc6, _pc7 = st.columns([2, 2, 2, 2])
            with _pc4:
                _snaps_default = 50 if _is_season else 200
                _snaps_max     = 200 if _is_season else 600
                _snaps_step    = 25  if _is_season else 50
                _min_snaps = st.slider(T["pro_min_snaps"], 0, _snaps_max,
                                       _snaps_default, _snaps_step,
                                       help=T["pro_min_snaps_help"])
            with _pc5:
                _pos_sel = st.multiselect(T["pro_pos"], ["DE", "OLB"],
                                          default=["DE", "OLB"])
            with _pc6:
                _yr_range = st.slider(T["pro_draft_yr"], 2006, 2024, (2008, 2022))
            with _pc7:
                _color_opts = [T["pro_color_yr"], T["pro_color_pos"]]
                if _is_season:
                    _color_opts.append("Season")
                _color_by = st.radio(T["pro_color_by"], _color_opts, horizontal=True)

            # ── データ準備（モード別） ─────────────────────────────────────────────
            if _is_season:
                _df_src    = _pr_season.copy()
                _y_col, _y_lower = _PPM_SEASON[_y_label]
                _snaps_col = "pass_rush_snaps"
            else:
                _df_src    = _pr_merged.copy()
                _y_col, _y_lower = _PPM[_y_label]
                _snaps_col = "total_pr_snaps"

            _x_c_col, _x_p_col = _ECM[_x_label]

            # X 値解決
            if _x_c_col and _x_c_col in _df_src.columns:
                _df_src["_x"] = _df_src[_x_c_col]
                if _use_pd_pro and _x_p_col and _x_p_col in _df_src.columns:
                    _df_src["_x"] = _df_src["_x"].combine_first(_df_src[_x_p_col])
            else:
                _df_src["_x"] = np.nan

            _df_src["_y"] = _df_src[_y_col] if _y_col in _df_src.columns else np.nan

            _pos_f = _pos_sel if _pos_sel else ["DE", "OLB"]
            _df = _df_src[
                _df_src["_x"].notna() &
                _df_src["_y"].notna() &
                (_df_src["_y"] > 0) &
                (_df_src[_snaps_col] >= _min_snaps) &
                _df_src["position"].isin(_pos_f) &
                _df_src["year"].between(_yr_range[0], _yr_range[1])
            ].copy()

            _n_pro = len(_df)

            if _n_pro < 5:
                st.warning(T["pro_no_data"].format(n=_n_pro))
            else:
                _xa = _df["_x"].values.astype(float)
                _ya = _df["_y"].values.astype(float)

                # OLS 回帰
                _slope, _intercept, _r, _pv, _se = stats.linregress(_xa, _ya)
                _r2 = _r ** 2
                _x_med = float(np.median(_xa))
                _y_med = float(np.median(_ya))
                ...
                if _is_season:
                    ...
                    _labels = {
                        "_x": _x_label, "_y": _y_label,
                        "year": "Draft Year", "season": "Season",
                        "position": "Position",
                        "pass_rush_snaps": "PR Snaps",
                        "qb_pressures": "QB Press.",
                        "sacks": "Sacks", "qb_pressure_rate": "Press. Rate",
                        "draft_round": "Round",
                    }
                else:
                    _hover_data = {
                    ...
                    _labels = {
                        "_x": _x_label, "_y": _y_label,
                        "year": "Draft Year", "position": "Position",
                        "total_pr_snaps": "PR Snaps (career)",
                        "total_qbp": "QB Press. (career)",
                        "total_sacks": "Sacks (career)",
                        "career_qbpr": "Press. Rate",
                        "seasons": "Seasons", "draft_round": "Round",
                    }

                # カラーモード
                if _color_by == T["pro_color_yr"]:
                    _fig_pro = px.scatter(
                        _df, x="_x", y="_y",
                        color="year",
                        color_continuous_scale="Plasma",
                        size=_snaps_col, size_max=20,
                        hover_name="_match_name",
                        hover_data=_hover_data,
                        labels=_labels,
                    )
                    _fig_pro.update_coloraxes(
                        colorbar=dict(
                            title=dict(text="Draft Year",
                                       font=dict(color=THEME["label"], size=11)),
                            thickness=12, len=0.55,
                            tickfont=dict(color=THEME["muted"], size=10),
                        )
                    )
                elif _is_season and _color_by == "Season":
                    _fig_pro = px.scatter(
                        _df, x="_x", y="_y",
                        color="season",
                    ...
                    _fig_pro = px.scatter(
                        _df, x="_x", y="_y",
                        color="position",
                        color_discrete_map={"DE": POS_COLORS["Edge"], "OLB": THEME["accent"]},
                        size=_snaps_col, size_max=20,
                        hover_name="_match_name",
                        hover_data=_hover_data,
                        labels=_labels,
                ...
                # 回帰直線
                _xl = np.linspace(_xa.min(), _xa.max(), 200)
                _yl = _slope * _xl + _intercept
                _fig_pro.add_trace(go.Scatter(
                ...
                # 95% CI バンド
                _Sxx = np.sum((_xa - _xa.mean()) ** 2)
                _ci  = 1.96 * _se * np.sqrt(1.0 / _n_pro + (_xl - _xa.mean()) ** 2 / _Sxx)
                _fig_pro.add_trace(go.Scatter(
                    x=np.concatenate([_xl, _xl[::-1]]),
                    y=np.concatenate([_yl + _ci, (_yl - _ci)[::-1]]),
                    fill="toself",
                ...
                _fig_pro.add_hline(
                    y=_y_med,
                    line=dict(color=THEME["grid"], width=1.2, dash="dot"),
                    annotation_text=f"med {_y_med:.3f}",
                    annotation_position="right",
                ...
                _fig_pro.add_vline(
                    x=_x_med,
                    line=dict(color=THEME["grid"], width=1.2, dash="dot"),
                    annotation_text=f"med {_x_med:.3f}",
                    annotation_position="top right",
                    annotation_font=dict(size=10, color=THEME["muted"]),
                )

                # 4象限ラベル
                _dx = (_xa.max() - _xa.min()) * 0.02
                _dy = (_ya.max() - _ya.min()) * 0.03
                _q_labels = [
                    (_x_med + _dx, _ya.max() - _dy, "✅ High–High", "#43A047", "left"),
                    (_xa.min() + _dx, _ya.max() - _dy, "⚠️ Low–High", THEME["muted"], "left"),
                    (_xa.min() + _dx, _ya.min() + _dy, "❌ Low–Low", "#E53935", "left"),
                    (_x_med + _dx, _ya.min() + _dy, "ℹ️ High–Low", THEME["muted"], "left"),
                ]
                for _qx, _qy, _qt, _qc, _qa in _q_labels:
                    _fig_pro.add_annotation(
                        x=_qx, y=_qy, text=_qt, showarrow=False,
                        font=dict(size=9, color=_qc), xanchor=_qa,
                    )

                _fig_pro.update_layout(**make_plot_layout(
                    height=580,
                    margin=dict(t=30, b=50, l=60, r=30),
                ))
                _fig_pro.update_xaxes(title_text=_x_label)
                _fig_pro.update_yaxes(title_text=_y_label)

                st.plotly_chart(_fig_pro, use_container_width=True)

                # ── 統計サマリー ──────────────────────────────────────────────
                _sig_delta = T["pro_sig"] if _pv < 0.05 else T["pro_not_sig"]
                _sc1, _sc2, _sc3, _sc4, _sc5 = st.columns(5)
                _sc1.metric(T["pro_n"],     f"{_n_pro}")
                _sc2.metric(T["pro_r"],     f"{_r:+.3f}")
                _sc3.metric(T["pro_r2"],    f"{_r2:.3f}")
                _sc4.metric(T["pro_pval"],  f"{_pv:.4f}", delta=_sig_delta)
                _sc5.metric(T["pro_slope"], f"{_slope:+.4f}")

                st.markdown("---")

                # ── 閾値分析 ──────────────────────────────────────────────────
                with st.expander(T["pro_thresh_hdr"], expanded=True):
                    st.caption(T["pro_thresh_note"])

                    _x_min_v = float(_xa.min())
                    _x_max_v = float(_xa.max())
                    _x_step  = max((_x_max_v - _x_min_v) / 40, 0.001)
                    _threshold = st.slider(
                        T["pro_thresh_slider"],
                        min_value=round(_x_min_v, 3),
                        max_value=round(_x_max_v, 3),
                        value=round(_x_med, 3),
                        step=round(_x_step, 3),
                        format="%.3f",
                    )

                    _above = _df[_df["_x"] >= _threshold]
                    _below = _df[_df["_x"] <  _threshold]

                    # 「成功」= Y が上位 50%（↓の場合は下位 50% が「速い」＝成功）
                    def _success_rate(subset: pd.DataFrame) -> float:
                        if len(subset) == 0:
                            return 0.0
                        if _y_lower:
                            return float((subset["_y"] <= _y_med).mean() * 100)
                        return float((subset["_y"] >= _y_med).mean() * 100)

                    _above_sr = _success_rate(_above)
                    _below_sr = _success_rate(_below)
                    _diff_sr  = _above_sr - _below_sr

                    _icon = lambda r: "✅" if r >= 60 else ("⚠️" if r >= 45 else "❌")
                    _crit = f"Y {'≤' if _y_lower else '≥'} median"

                    _tc1, _tc2 = st.columns(2)
                    with _tc1:
                        st.markdown(f"""<div class="metric-box">
    <b>{_x_label} ≥ {_threshold:.3f}</b><br>
    {T['pro_players']}: <b>{len(_above)}</b><br>
    {T['pro_avg']} {_y_label.split('(')[0].strip()}: <b>{_above['_y'].mean():.4f}</b><br>
    {T['pro_med']}: <b>{_above['_y'].median():.4f}</b><br>
    {T['pro_success']} ({_crit}):
    <b>{_icon(_above_sr)} {_above_sr:.1f}%</b>
    </div>""", unsafe_allow_html=True)
                    with _tc2:
                        st.markdown(f"""<div class="metric-box">
    <b>{_x_label} < {_threshold:.3f}</b><br>
    {T['pro_players']}: <b>{len(_below)}</b><br>
    {T['pro_avg']} {_y_label.split('(')[0].strip()}: <b>{_below['_y'].mean():.4f}</b><br>
    {T['pro_med']}: <b>{_below['_y'].median():.4f}</b><br>
    {T['pro_success']} ({_crit}):
    <b>{_icon(_below_sr)} {_below_sr:.1f}%</b>
    </div>""", unsafe_allow_html=True)

                    # 結論文
                    _conclusion_txt = T["pro_conclusion"](
                        _x_label, _threshold, _diff_sr, "higher" if _diff_sr > 0 else "lower"
                    )
                    st.markdown(f"> {_conclusion_txt}")

                    # 棒グラフで比較
                    _bar_fig = go.Figure()
                    _bar_x = [f"≥ {_threshold:.3f}\n(n={len(_above)})",
                              f"< {_threshold:.3f}\n(n={len(_below)})"]
                    _bar_y = [_above_sr, _below_sr]
                    _bar_colors = [
                        "#43A047" if _above_sr >= 55 else "#E53935" if _above_sr < 45 else "#FB8C00",
                        "#43A047" if _below_sr >= 55 else "#E53935" if _below_sr < 45 else "#FB8C00",
                    ]
                    _bar_fig.add_trace(go.Bar(
                        x=_bar_x, y=_bar_y,
                        marker_color=_bar_colors,
                        marker_line=dict(width=0),
                        text=[f"{v:.1f}%" for v in _bar_y],
                        textposition="outside",
                        textfont=dict(color=THEME["text_bright"], size=13, family=THEME["font"]),
                        width=0.45,
                    ))
                    _bar_fig.add_hline(
                        y=50, line=dict(color=THEME["muted"], width=1, dash="dot"),
                        annotation_text="50% (random)", annotation_position="right",
                        annotation_font=dict(size=10, color=THEME["muted"]),
                    )
                    _bar_fig.update_layout(**make_plot_layout(
                        height=280,
                        margin=dict(t=20, b=40, l=50, r=80),
                        showlegend=False,
                        yaxis=dict(
                            range=[0, max(max(_bar_y) * 1.2, 65)],
                            ticksuffix="%",
                            title_text=T["pro_success"],
                        ),
                    ))
                    st.plotly_chart(_bar_fig, use_container_width=True)

                # ── データテーブル ────────────────────────────────────────────
                with st.expander(T["pro_table_hdr"], expanded=False):
                    if _is_season:
                        _tbl_cols = [
                            "_match_name", "year", "season", "position", "college",
                            "draft_round", "_x", "_y",
                            "pass_rush_snaps", "qb_pressures", "qb_pressure_rate",
                            "sacks", "get_off_time", "time_to_pressure",
                        ]
                        _rename = {
                            "_match_name":      "Name",
                            "year":             "Draft Year",
                            ...
                            "pass_rush_snaps":  "PR Snaps",
                            "qb_pressures":     "QB Press.",
                            "qb_pressure_rate": "Press. Rate",
                            "sacks":            "Sacks",
                            "get_off_time":     "Get-Off (s)",
                            "time_to_pressure": "TTP (s)",
                        }
                    else:
                        _tbl_cols = [
                            "_match_name", "year", "position", "college",
                            "draft_round", "draft_pick",
                            "_x", "_y",
                            "total_pr_snaps", "total_qbp", "career_qbpr",
                            "total_sacks", "sacks_per_season",
                            "seasons", "season_first", "season_last",
                        ]
                        _rename = {
                            "_match_name":    "Name",
                            "year":           "Draft Year",
                            ...
                            "_x":             _x_label,
                            "_y":             _y_label,
                            "total_pr_snaps": "PR Snaps",
                            "total_qbp":      "QB Press.",
                            "career_qbpr":    "Press. Rate",
                            "total_sacks":    "Sacks",
                            "sacks_per_season": "Sacks/Ssn",
                            "seasons":        "Seasons",
                            "season_first":   "First Yr",
                            "season_last":    "Last Yr",
                        }

                    _disp = _df[[c for c in _tbl_cols if c in _df.columns]].copy()
                    _disp = _disp.rename(columns=_rename)
                    _disp = _disp.sort_values(_y_label, ascending=_y_lower)

                    for _col in _disp.select_dtypes(include=["float64"]).columns:
                        if "Rate" in _col or "rate" in _col.lower():
                            _disp[_col] = _disp[_col].round(4)
                        else:
                            _disp[_col] = _disp[_col].round(2)

                    st.dataframe(_disp.reset_index(drop=True), use_container_width=True)

                    _csv_bytes = _disp.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        T["download_csv"],
                        _csv_bytes,
                        file_name=(
                            f"edge_rusher_"
                            f"{_x_label[:12].replace(' ', '_')}_vs_"
                            f"{_y_label[:12].replace(' ', '_')}.csv"
                        ),
                        mime="text/csv",
                    )
